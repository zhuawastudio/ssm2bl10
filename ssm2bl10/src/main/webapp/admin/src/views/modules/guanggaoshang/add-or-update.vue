...
export default {
  data() {
	...
	zhaopian : false,
      ...
      rules: {
          guanggaoshangzhanghao: [
                { required: true, message: '广告商账号不能为空', trigger: 'blur' },
          ],
          mima: [
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `guanggaoshang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.guanggaoshangCrossAddOrUpdateFlag = false;
                  this.parent.search();
