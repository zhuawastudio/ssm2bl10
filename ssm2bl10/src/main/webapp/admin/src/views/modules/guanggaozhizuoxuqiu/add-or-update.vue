<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="需求提交时间" prop="xuqiutijiaoshijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.xuqiutijiaoshijian" 
                type="datetime"
                placeholder="需求提交时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.xuqiutijiaoshijian" label="需求提交时间" prop="xuqiutijiaoshijian">
              <el-input v-model="ruleForm.xuqiutijiaoshijian" 
                placeholder="需求提交时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="制作需求内容" prop="zhizuoxuqiuneirong">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.zhizuoxuqiuneirong" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.zhizuoxuqiuneirong" label="制作需求内容" prop="zhizuoxuqiuneirong">
                    <span v-html="ruleForm.zhizuoxuqiuneirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	kehuzhanghao : false,
	kehuxingming : false,
	xuqiutijiaoshijian : false,
	zhizuoxuqiuneirong : false,
  ...
  created() {
	this.ruleForm.xuqiutijiaoshijian = this.getCurDateTime()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.kehuxingming = true;
          ...
          if(o=='xuqiutijiaoshijian'){
            this.ruleForm.xuqiutijiaoshijian = obj[o];
	    this.ro.xuqiutijiaoshijian = true;
            continue;
          }
          if(o=='zhizuoxuqiuneirong'){
            this.ruleForm.zhizuoxuqiuneirong = obj[o];
	    this.ro.zhizuoxuqiuneirong = true;
            continue;
          }
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.guanggaoshangzhanghao!=''&&json.guanggaoshangzhanghao){
              		this.ruleForm.guanggaoshangzhanghao = json.guanggaoshangzhanghao
		}
		if(json.guanggaoshangmingcheng!=''&&json.guanggaoshangmingcheng){
              		this.ruleForm.guanggaoshangmingcheng = json.guanggaoshangmingcheng
		}
    ...
    info(id) {
      this.$http({
        url: `guanggaozhizuoxuqiu/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.zhizuoxuqiuneirong = this.ruleForm.zhizuoxuqiuneirong.replace(reg,'../../../ssm2bl10/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `guanggaozhizuoxuqiu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.guanggaozhizuoxuqiuCrossAddOrUpdateFlag = false;
                  this.parent.search();
