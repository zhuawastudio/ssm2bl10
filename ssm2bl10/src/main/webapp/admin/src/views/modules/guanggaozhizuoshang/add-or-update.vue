...
export default {
  data() {
	...
	zhaopian : false,
      ...
      rules: {
          zhizuoshangzhanghao: [
                { required: true, message: '制作商账号不能为空', trigger: 'blur' },
          ],
          mima: [
                { required: true, message: '密码不能为空', trigger: 'blur' },
          ],
          zhizuoshangmingcheng: [
                { required: true, message: '制作商名称不能为空', trigger: 'blur' },
          ],
          fuzerenxingming: [
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `guanggaozhizuoshang/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `guanggaozhizuoshang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.guanggaozhizuoshangCrossAddOrUpdateFlag = false;
                  this.parent.search();
