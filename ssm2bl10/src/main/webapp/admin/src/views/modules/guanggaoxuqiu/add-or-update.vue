    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="需求内容" prop="xuqiuneirong">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.xuqiuneirong" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.xuqiuneirong" label="需求内容" prop="xuqiuneirong">
                    <span v-html="ruleForm.xuqiuneirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.guanggaoyaoqiu = true;
          ...
          if(o=='xuqiuneirong'){
            this.ruleForm.xuqiuneirong = obj[o];
	    this.ro.xuqiuneirong = true;
            continue;
          }
          if(o=='fabushijian'){
	...
	this.ruleForm.xuqiuneirong = this.ruleForm.xuqiuneirong.replace(reg,'../../../ssm2bl10/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `guanggaoxuqiu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.guanggaoxuqiuCrossAddOrUpdateFlag = false;
                  this.parent.search();
