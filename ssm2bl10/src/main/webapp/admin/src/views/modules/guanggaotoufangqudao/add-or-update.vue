<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="投放预算要求" prop="toufangyusuanyaoqiu">
          <el-input v-model="ruleForm.toufangyusuanyaoqiu" 
              placeholder="投放预算要求" clearable  :readonly="ro.toufangyusuanyaoqiu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="投放预算要求" prop="toufangyusuanyaoqiu">
              <el-input v-model="ruleForm.toufangyusuanyaoqiu" 
                placeholder="投放预算要求" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.toufangqudao = true;
          ...
          if(o=='toufangyusuanyaoqiu'){
            this.ruleForm.toufangyusuanyaoqiu = obj[o];
	    this.ro.toufangyusuanyaoqiu = true;
            continue;
          }
          if(o=='zhuyishixiang'){
    ...
    info(id) {
      this.$http({
        url: `guanggaotoufangqudao/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `guanggaotoufangqudao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.guanggaotoufangqudaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
